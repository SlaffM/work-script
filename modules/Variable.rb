class Variable
		...
		def create_headers(variable)			
			...
			@signal_name 		= get_signal_name(tag_name)						#навзвание сигнала
		...
		def get_voltage_level(tag_name)
			tag_name[0, 8].nil? ? "" : tag_name[0, 8].strip
		end
		def get_vl_name(tag_name)
			tag_name[8, 27].nil? ? "" : tag_name[8, 27].strip
		end
		def get_panel(tag_name)
			tag_name[35, 8].nil? ? "" : tag_name[35, 8].strip
		end
		def get_system(tag_name)
			tag_name[43, 8].nil? ? "" : tag_name[43, 8].strip
		end
		def get_ka_name(tag_name)
			tag_name[51, 27].nil? ? "" : clear_mip_tag(tag_name[51, 27])
		end
		def get_signal_name(tag_name)
			tag_name[78, 48].nil? ? "" : tag_name[78, 48].strip
		end
		def get_symbaddr(name)
			name.nil? ? "" : name
		end
		...
		def get_prefix_device_name
			@variable_name.include?(".") ? @variable_name[0..@variable_name.rindex(".")-1] : ""
		end
